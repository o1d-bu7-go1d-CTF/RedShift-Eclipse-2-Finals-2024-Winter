            ...
            async with session.get(f'http://{HOST}:{PORT}/doctors', params={'search': search_query}, cookies=cookies) as resp:
                if resp.status == 200:
                    ...
                    if len(doctors) == expected_count:
                        print(Fore.GREEN + f"[+] Search query '{search_query}': Expected {expected_count} results, found {len(doctors)}." + Style.RESET_ALL)
                        return 101, None
                    else:
                        print(Fore.RED + f"[-] Search query '{search_query}': Expected {expected_count} results, found {len(doctors)}." + Style.RESET_ALL)
                        await delete_user_from_db(email)
                        return 104, None
                else:
                    print(Fore.RED + f"[-] Error during search query '{search_query}'" + Style.RESET_ALL)
                    await delete_user_from_db(email)
                    return 104, None
