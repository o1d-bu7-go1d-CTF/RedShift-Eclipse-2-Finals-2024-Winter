...
async def check_user_role_change(ip, port, username, passwd):
    try:
        ...
        connection.recvuntil("Введите имя пользователя: ".encode('utf-8'), timeout=5)
        connection.sendline("admin".encode('utf-8'))
        connection.recvuntil("Введите пароль: ".encode('utf-8'), timeout=5)
        connection.sendline("admin".encode('utf-8'))
        ...
        connection.recvuntil("Введите роль (operator/admin): ".encode('utf-8'), timeout=5)
        connection.sendline("admin".encode('utf-8'))
        response = connection.recvuntil("> ".encode('utf-8'), timeout=5)
        ...
        connection.recvuntil("> ".encode('utf-8'), timeout=5)
        connection.sendline("update_user_role".encode('utf-8'))
        connection.recvuntil("Введите имя пользователя для обновления роли: ".encode('utf-8'), timeout=5)
        connection.sendline(username.encode('utf-8'))
        connection.recvuntil("Введите новую роль (operator/admin): ".encode('utf-8'), timeout=5)
        ...
        connection.recvuntil("Введите имя пользователя для удаления: ".encode('utf-8'), timeout=5)
        connection.sendline(username.encode('utf-8'))
        response = connection.recvuntil("> ".encode('utf-8'), timeout=5)
