		...
		async with aiohttp.ClientSession() as session:
			async with session.get(url) as check_connection:
				if check_connection.status == 200:
					print(Fore.GREEN + f"[+] Checker (Keeyb): {HOST}:{PORT} - connection successful" + Style.RESET_ALL)
					return 101, None
	except:
		print(Fore.RED + f"[-] Checker (Keeyb): {HOST}:{PORT} - connection lost" + Style.RESET_ALL)
		return 104, None
		...
		async with aiohttp.ClientSession() as session:
			...
			async with session.post(url, data=reg_data) as user_register:
				if user_register.status == 200 or user_register.status == 302:
					print(Fore.GREEN + f"[+] Checker (Keeyb): {HOST}:{PORT} - registration successful" + Style.RESET_ALL)
					return 101, None
				else:
					return 104, None
	except:
		print(Fore.RED + f"[-] Checker (Keeyb): {HOST}:{PORT} - registration failed" + Style.RESET_ALL)
		return 104, None