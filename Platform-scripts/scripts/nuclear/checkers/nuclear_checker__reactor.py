...
async def check_reactor_commands(ip, port):
    try:
        ...
        response = connection.recvuntil("Введите новое положение управляющих стержней (0-100%): ".encode('utf-8'), timeout=10)
        if response:
        ...
        response = connection.recvuntil("Введите количество охлаждающей жидкости: ".encode('utf-8'), timeout=10)
        if response:
